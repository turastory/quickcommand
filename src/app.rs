...
impl Ui for SystemUi {
    ...
    fn ask_choice(
        ...
        loop {
            ...
            let answer = self.ask_input("Choose a number or type your own answer", default_label)?;
...
fn run_task_from_cli(cli: Cli, output_mode: OutputMode, ui: &mut impl Ui) -> Result<i32> {
    ...
    match run_task_with_deps(&backend, ui, &mut executor, &task, &resolved, &context, output_mode)? {
...
pub fn run_task_with_deps<B: Backend, U: Ui, E: Executor>(
    ...
    loop {
        ...
        match backend.generate(&request, context)? {
            ModelReply::Command(reply) => match config.mode {
                Mode::Copy => match output_mode {
                    OutputMode::EmitCommand => return Ok(TaskResult::EmittedCommand(reply.command)),
                    OutputMode::Human => {
                ...
                Mode::Execute => {
                    ...
                    if is_destructive(&reply.command)
                        && !ui.ask_yes_no(
                            "Warning: this command looks destructive. Run it?",
                            false,
                        )?
                    {
...
mod tests {
    ...
    struct FakeBackend {
        replies: std::cell::RefCell<Vec<ModelReply>>,
    }
    ...
    fn human_copy_mode_prints_command_without_executing() {
        let backend = FakeBackend {
            replies: std::cell::RefCell::new(vec![ModelReply::Command(CommandReply {
